...
public class PackingListsController : BaseController
{
    ...
    public async Task<ActionResult> RemovePackingList([FromRoute] PackItem command)
    {
        await _commandDispatcher.DispatchAsync(command);
        return Ok();
    }
    ...
    [HttpDelete("{id:guid}")]
    public async Task<ActionResult> DeletePackingList([FromRoute] RemovePackingList command)
    {
        await _commandDispatcher.DispatchAsync(command);
        return Ok();
    }
